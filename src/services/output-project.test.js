...
describe("createOutputProject function", () => {
  ...
  beforeEach(() => {
    ...
    fs.writeFile.mockImplementation(
        (file, content) => { dictionary[file] = content; });
    path.resolve.mockImplementation((_path, folder) => folder);
  });

  test(
      "should create files: src, test and exercise if 'mustHaveTest' is 'TRUE'",
      async () => {
  ...
  test("should create files: src and exercise if 'mustHaveTest' is 'FALSE'",
       async () => {
