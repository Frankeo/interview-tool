 ...
 */
const createOutputProject = async (
  { testContent, instructionsContent },
  hasTests
) => {
  ...
  await fs.writeFile(
    `${outputFolderPath}/${EXERCISE_FILE}`,
    instructionsContent
  );
};
...
const createOutputFolder = async (hasOutputInExecPath, projectName) => {
  ...
  const outputDirectoryPath = path.resolve(
    hasOutputInExecPath ? cwd() : mainDirectoryPath,
 ...
 * @param  {string} projectName
 */
const addProjectConfig = async (
  outputDirectoryPath,
  topic,
  difficulty,
  projectName
) => {
  const jsonContent = await fs.readJSON(
    `${outputDirectoryPath}/${PACKAGE_JSON_FILE}`
  );
